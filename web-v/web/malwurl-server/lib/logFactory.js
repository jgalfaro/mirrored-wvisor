...
var logFactory = function(){
   ...
   * @param {Boolean} [stdout] - the logger prints
   * to the standard output
   * @param {String} [file] - a file path if present
   * to write the logs into
   */
  self.init = function(level, stdout, file){
    ...
    if(file && file.trim().length > 0){
      filtered.pipe(br).pipe(
        fs.createWriteStream(file, {flag : "a"}));
    }
