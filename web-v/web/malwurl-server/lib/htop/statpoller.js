...
* @param {Array} [options.pollers] - the pollers to load,
* all by default
*/
...
StatPoller.prototype.start = function(){
  ...
  _.forEach(require("./poller").Pollers,
    function itPoller(poller, name){
    //filter to get only the requested
    if(self.options.pollers === undefined
      || _.contains(self.options.pollers, name)){
  ...
  pollingDomain.on("error", function pollingDomainError(err){
    logger.error(util.format("%s : %s",
      err.domain.type, err.message));
    ...
    _.forEach(self.pollers, function(poller, name){
      var handler = setInterval(function(){
        ...
        poller.poll(function(infos){
           ...
           * stat polled (the name of the poller)
