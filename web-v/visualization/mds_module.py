...
def _smacof_single(similarities, metric=True, n_components=2, init=None,
    ...
    for it in range(max_iter):
        ...
        #stress = sqrt(((dis.ravel() - disparities.ravel()) ** 2).sum())
        #stress = sqrt(((dis.ravel() - disparities.ravel()) ** 2).sum() / (dis.ravel() ** 2).sum())
        stress = sqrt(((dis.ravel() - disparities.ravel()) ** 2).sum() / (disparities.ravel() ** 2).sum())
